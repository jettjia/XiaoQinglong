  ...
  const loadModels = React.useCallback(async () => {
    try {
      ...
      const mapped = data.map((m: any) => ({
        ...
        baseUrl: m.baseUrl,
        apiKey: m.api_key,
        status: (m.status === '1' ? 'active' : 'configured') as 'active' | 'configured' | 'error',
        latency: m.latency || 'N/A',
        contextWindow: m.contextWindow || 'N/A',
        usage: m.usage || 0,
        type: m.modelType as 'llm' | 'embedding',
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6">
            ...
            <div className="space-y-3">
              {configuredProviders.map(provider => (
                <div key={provider} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                  <span className="text-xs font-medium text-slate-700">{provider}</span>
                  <CheckCircle2 size={14} className="text-green-500" />
                </div>
              ))}
              {configuredProviders.length === 0 && (
                <p className="text-xs text-slate-400 text-center py-2">{t('models.noProviders')}</p>
              )}
            </div>
          </div>
        </div>
