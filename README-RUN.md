...
```
├── backend
│   ├── agent-frame agent相关的业务，前端调用此，需要运行ai。它再调用runner。配置文件在：backend/agent-frame/manifest/config/config.yaml。DDD架构设计
│   └── runner 执行器，可以运行多个实例,配置文件在：开发：dev-skills-config.yaml， 生产：skills-config.yaml
├── skills
│   ├── 运行的skill,遵循 agent-skill标准
├── frontend
│   ├── agent-ui agent 前端代码，与agent-frame交互
├── mock
│   ├── a2a 模拟第三方的a2a服务
